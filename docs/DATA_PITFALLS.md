...
### 픽스
`naver_quote(key, settled=True)`가 `fxlist`도 `/prices` 일별 정산 바를 먼저 읽도록 확장.
**단 이때 카테고리를 틀리면 조용히 실패한다 — 아래 항목 9 참조.**
...
## 9. 네이버 `/prices` 일별 바 카테고리 — 틀리면 404가 아니라 **빈 배열**

### 규칙
`fxlist` 코드의 일별 바 카테고리는 코드마다 다르다. `naver_macro._FX_PRICES_CATS`가
source of truth이며, `naver_quote`/`naver_settled_date` 둘 다 이 표를 탄다.

| 코드 | `/prices` 카테고리 |
|---|---|
| `.DXY`, `FX_USDKRW` | `marketindex/**exchange**/{code}/prices` |
| `EURUSD` (통화쌍) | `marketindex/**exchangeWorld**/{code}/prices` |

### 왜 (2026-07-22 사고)
항목 4의 픽스가 `EURUSD`도 `exchange/EURUSD/prices`로 읽었는데, 이 경로는 **404가 아니라
빈 배열 `[]`**을 돌려준다. 예외가 안 나니 조용히 `worldDailyQuote(FX_EURUSD)` 데스크
폴백으로 새고, 항목 7에서 이미 "쓰면 안 된다"고 배제한 **아시아-세션 클럭 시리즈**가
석 달 가까이 시트에 박혔다.

| 대상일 | 일별 바(진실, exchangeWorld) | 데스크(시트에 박힌 값) |
|---|---|---|
| 7/21 | 1.1398 / **-0.14%** | 1.1405 / **-0.01%** |
| 7/20 | 1.1414 / **-0.22%** | 1.1406 / **-0.29%** |
| 7/15 | 1.1460 / **+0.31%** | 1.1435 / **-0.08%** |

데스크 표엔 **토요일 행까지 0.00%로 들어있다** — 이게 보이면 그 시리즈는 NY 종가가 아니다.

### 교훈
- 빈 배열을 "이 소스엔 데이터가 없다"로 해석하고 폴백을 다는 건 **오답을 조용히 고르는 길**이다.
  새 코드를 `_DISPATCH`에 추가하면 `/prices`가 실제로 바를 돌려주는지 **1회 확인**할 것.
- 데스크(`worldDailyQuote`) 폴백은 `fxlist` settled 경로에서 **제거**했다(항목 7과 동일 이유).
  실패 시 `None`을 반환해 호출부가 yfinance 일별 바로 폴백한다.
- 검증: `naver_settled_date`가 `fxlist`도 지원한다. `daily_review`가 DXY·EURUSD 정산 바
  거래일을 target_date와 비교해 경고한다(2026-07-18 DXY가 07-16 바로 밀렸던 건 무경고였음).

---

## 10. 정산 바 게시 지연은 '경고'로 끝나면 안 된다 — 나중에 다시 확인해 패치

### 규칙
`settled_backfill.py`(워크플로우 `settled-backfill.yml`, ≈10:30·14:00 KST 화~토)가
글로벌 시트 3.매크로 동향의 원자재·FX 셀을 다시 확인해 패치한다. **멱등** —
`naver_settled_date(key) == target`이고 시트 값과 다를 때만 쓴다.

### 왜 (2026-07-22)
daily-global(≈22:36 UTC)이 찍은 로그:
```
정산 바 거래일: {'WTI':'2026-07-21', ..., 'COPPER':'2026-07-20'}
⚠️ 원자재 정산 바가 대상일(2026-07-21)보다 이전 — 전일 값일 수 있음: {'COPPER': '2026-07-20'}
```
LME 현물 구리 07-21 바(13,898/+2.04%)는 **배치가 끝난 뒤에 게시**됐다. 항목 8의 라이브
우선 가드도 이때는 라이브까지 D-1이라 못 막는다. 경고만 찍고 D-1(13,621/+0.89%)이
그대로 박혔고, 07-18 시트도 같은 증상이었다. **배치 시각을 조정해 해결할 수 없다 —
게시 시각이 일정하지 않다.**

### 주의
- 백필은 종합 경보(C-Risk·★매크로 종합)도 **시트에 표시된 값으로** 재계산한다.
  레그를 재수집하면 실행 시각의 다음 세션 값이 섞이므로 절대 그러지 말 것.
- `scouter_logger`(05:17 KST)는 daily-global보다 **더 이른** 시각이라 같은 지연에 노출된다.
  타임시리즈 쪽은 아직 백필 대상이 아니다(알려진 미결).
