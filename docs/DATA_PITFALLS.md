...
- `scouter_logger`(05:17 KST)는 daily-global보다 **더 이른** 시각이라 같은 지연에 더 크게
  노출된다. 타임시리즈는 append-only라 백필이 아니라 **게시 가드**로 막는다 — 항목 11.

---

## 11. 타임시리즈는 되돌릴 수 없다 — 정산 바 게시 **전엔 기록하지 말 것**

### 규칙
`scouter_logger`는 `collect_macro_scores(settled=True)`를 부르기 **전에**
`late_settled_bars(target)`로 WTI·BRENT·GOLD·SILVER·COPPER 정산 바 거래일을 확인한다.
하나라도 대상 세션 바가 미게시면 **fetch도 하지 않고 `return 0`** — 뒤 회차가 기록한다.

### 왜 (2026-07-23 사고)
`스카우터_매크로_타임시리즈`의 `2026-07-23 05:17` 행(07-22 세션)이 **행 전체 D-1**이었다.

| 입력 | 기록된 값(07-21 바) | 실제 07-22 정산 |
|---|---|---|
| WTI | 84.34 / +2.26% | 86.83 / +2.95% |
| Brent | 91.01 / +2.01% | 94.07 / +3.36% |
| GOLD | 4,076.4 / +1.51% | 4,151.9 / +1.85% |
| COPPER | 13,898 / +2.04% | 13,812.7 / −0.62% |
| VIX | 19.49 | 16.64 |

결과: **C-RISK 17.4(실제 29.4), VIX점수 26.7(16.6), S-RISK 33.2(17.4)** — 부호까지 뒤집힘.
직전 07-22 행도 구리가 07-20 바(13,621/+0.89%)로 같은 증상이었다.

방아쇠는 **매시 :17에 도는 외부 `workflow_dispatch` 루프**였다. cron(`17 20 * * 1-5`)은
GH 지연으로 보통 21:30Z에 돌아 우연히 정확했는데, 정시에 뜬 dispatch(20:17Z = 종가
**17분 후**)가 22h 가드를 먼저 통과해 기록을 선점했다.

### 교훈
- **항목 10(시트)과 달리 백필로 못 고친다.** 타임시리즈는 append-only이고 하루 1회
  가드(22h) 때문에, 뒤늦게 정상값을 얻은 실행이 이미 박힌 행을 덮어쓰지 못한다.
  **틀린 값을 안 쓰는 것**만이 방법이다.
- 그래서 스케줄을 **3회차**(≈05:17·09:30·13:00 KST)로 늘렸다. 1차는 스킵되는 게 정상이다.
  게시 시각이 일정하지 않아 "한 번 더 늦게"로는 못 맞춘다(항목 10과 같은 이유).
- `target_session_date`는 `settled_backfill`과 **중복 정의**다 — 한쪽만 고치지 말 것.
- 디버그로 강행하려면 `FORCE_APPEND=1`(워크플로우 `force_append` 입력)이 가드도 뚫는다.

### 검증
```
python -c "import scouter_logger as s,datetime as d; print(s.late_settled_bars(s.target_session_date(d.datetime.now())))"
```
`{}`면 기록 가능한 시각, 값이 있으면 그 지표가 아직 미게시다.
