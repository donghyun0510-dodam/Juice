...
모바일 목록에 없어 데스크톱 worldDailyQuote 사용 — 단 이건 **아시아-세션 클럭**이라
NY 종가와 부호까지 어긋날 수 있다(2026-07-15 USD/JPY +0.11% vs NY 종가 -0.15%).
그래서 settled=True(마감 후 배치)면 fxdesk는 None을 반환해 호출부가 yfinance(NY
종가)로 폴백하게 한다(라이브/대시보드는 종전대로 worldDailyQuote).
"""
import re
import time
import threading
...
# 현물(캐시) 금속 — 라이브 아이템이 곧 당일 정산값(marketStatus=null, localTradedAt=
# 캐시 픽싱). 선물(GC/SI cv1)과 달리 라이브에 '다음 세션' 오염이 없다. settled 경로에서
# /prices 일별 바 게시가 지연될 때 라이브를 정산값으로 신뢰하기 위한 집합. (2026-07-15
...
def naver_quote(key, settled=False):
    ...
    strat, code = spec
    try:
        if settled and strat == "metals" and code in _SPOT_METALS:
            # 구리 현물(CMCU0): LME 캐시라 라이브가 이미 당일 정산값인데 /prices 일별
            # 바는 게시가 수 시간 지연돼 배치 실행 시각(≈18:37 ET)에 아직 D-1 바만
            # 있을 때가 있다. 라이브·일별 바 중 거래일(localTradedAt)이 늦은 쪽을 쓴다.
            item = _get_json(f"{_API}/marketindex/metals/{code}")
            live = _from_item(item)
            live_date = (item.get("localTradedAt") or "")[:10] if isinstance(item, dict) else ""
            bars = _prices_bars("metals", code)
            daily = _from_item(bars[0]) if bars else (None, "", None)
            daily_date = (bars[0].get("localTradedAt") or "")[:10] if bars else ""
            if live[0] is not None and (daily[0] is None or live_date >= daily_date):
                return live
            return daily
        ...
        if strat == "fxdesk":
            if settled:
                # worldDailyQuote(USD/JPY·USD/CNY)는 NY 종가 스냅샷이 아니라 아시아-세션
                # 클럭이라, NY 종가 기준 마감 후 배치와 부호까지 어긋난다(2026-07-15
                # USD/JPY 데스크 +0.11% vs 실제 NY 종가 -0.15%). settled 배치는 None을
                # 반환해 호출부가 yfinance(JPY=X/CNY=X) 일별 NY-종가 바로 폴백하게 한다.
                # 라이브(settled=False, 대시보드)는 종전대로 worldDailyQuote 사용.
                return None, "", None
...
def naver_settled_date(key):
    ...
    # 검증용 거래일도 라이브·일별 바 중 늦은 쪽으로 맞춘다.
    if strat == "metals" and code in _SPOT_METALS:
        item = _get_json(f"{_API}/marketindex/metals/{code}")
        live_date = (item.get("localTradedAt") or "")[:10] if isinstance(item, dict) else None
        cands = [d for d in (bar_date, live_date) if d]
