...
형식: `- YYYY-MM-DD [KR|US|THEMATIC] [제목](파일명.md) — 규칙명 2~3개 쉼표 나열`

**한 줄은 150자 이내.** 상세(draft↔발행 원문 대조·맥락)는 lesson 파일에 이미 있으므로 인덱스엔 **무엇을 배웠는지 이름표만**. 문단을 쓰지 말 것 — 인덱스는 목차이지 요약본이 아니다.
...
이 파일은 **작성 규칙집**이지 학습 로그가 아니다. blog-kr/blog-us/blog-thematic이 매 초안마다 통째로 읽으므로, 커지면 그만큼 매번 비용이 든다.

**작업 순서 (반드시 이 순서)**
1. **먼저 기존 가이드를 읽는다.** 이번 lesson의 각 패턴이 **기존 규칙과 같은 원칙인지** 판정한다.
2. **같은 원칙이면 → 병합.** 새 bullet을 만들지 말고 기존 규칙 문장에 흡수한다(예시가 더 선명하면 예시만 교체, 예외 조건이면 그 규칙 안에 단서로 추가). *"…의 US 버전"·"…의 재확인"·"line NN의 강화" 같은 항목을 새로 만드는 것이 바로 append-only 실패다.*
3. **진짜 새 원칙일 때만 신규 등재.** 기존 어느 규칙으로도 설명되지 않는 경우에만.
4. 규칙이 무효화됐으면 삭제한다.

**하드 제약**
- **상한: bullet 120개 / 25,000자.** 초과하면 신규 등재 전에 압축을 먼저 수행한다(비슷한 규칙 병합, 예시 축약).
- **가이드엔 출처 태그(`[일일 리뷰, US, 2026-07-07]`)·draft↔발행 원문 대조를 넣지 않는다.** 그건 lesson 파일 몫. 가이드엔 **규칙 + 짧은 예시 하나**만.
- **현재 구조(0.진정성 / 1.장세별 표 / 2.일일 리뷰 / 3.테마 / 4.중복 배치 표 / 5.어휘 치환 표 / 6.신호 위계)를 유지한다.** "삭제/어조/표현/구조/사실추가" 5분류로 되돌리지 말 것 — 하나의 원칙이 5곳에 흩어져 중복을 낳았던 옛 구조다.
- **"강세장 한정"·"대급락일 예외" 같은 조건절은 지우지 말고 §1 장세 표의 해당 칸에 넣는다.** 예외를 없애는 게 아니라 한눈에 보이게 만드는 것이 목적.
- 새 패턴이 §0 진정성과 충돌하면(그럴듯한 인과 서사 강화 등) **채택하지 않는다.**
...
- **단, 갱신 = 병합이지 추가가 아니다.** 2026-07-10 압축 전 가이드는 317개 규칙·78,246자까지 불어났는데, 그중 97개가 같은 원칙 7개를 다시 쓴 것이었다(초안마다 35k 토큰). 새 항목을 만들기 전에 **"이건 기존 어느 규칙의 사례인가?"를 먼저 물을 것.**
