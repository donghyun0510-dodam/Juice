...
# 네이버 매크로/위험심리 1차 소스 (실패 시 yfinance/CNBC/investing 폴백)
...
def _yf_yield(yf_ticker):
    ...
    2YY=F 등은 며칠씩 동일값으로 stale되는 경우가 있어 stale은 폴백에서 제외."""
    try:
...
def get_yield_live(maturity, yf_ticker):
    """네이버 모바일 미국채 primary → CNBC → 미 재무부 par yield → yfinance 폴백."""
